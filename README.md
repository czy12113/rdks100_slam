...
| SLAM 建图 | Cartographer 2D/3D 建图（v7.0 真实接入 + v8.0 前倾40° TF 补偿 + v8.2 GPT修改版建图稳定性优化）、占据栅格地图、轨迹、地图保存/加载 |
...
- `pointcloud_to_laserscan.yaml` 默认切片从 `-1.5m~1.2m` 收窄为 `-0.20m~0.35m`，`range_max` 调整为 `15.0m`，减少地面、桌腿、门框上沿、多层高度点投影到同一 2D 扫描面导致的双墙/重影。
...
## SLAM 建图（v7.0 新增，v8.2 GPT修改版优化）

### 概述

`czybot_slam` 功能包实现了 Cartographer SLAM 建图，支持 2D 和 3D 两种模式，以及 slam_toolbox 备选方案。v8.2 GPT修改版重点优化 2D 建图重影问题：雷达外参可通过 launch 参数现场微调，点云转 LaserScan 使用窄高度切片，Cartographer 2D 参数更保守，`scan_dedup.py` 支持时间戳策略 A/B 测试。

### 数据流（2D Cartographer，推荐）

```
Mid-360S → /livox/lidar (PointCloud2, 10Hz, livox_frame坐标系，有40°前倾)
  → pointcloud_to_laserscan（base_link坐标系下默认窄切片 -0.20m~0.35m，range_max=15m）
    → /scan (LaserScan)
      → scan_dedup.py（限频 + 可选时间戳单调修复，发布统计日志）
        → /scan_dedup
          → cartographer_node（2D 纯激光扫描匹配，暂不默认融合未标定 STM32 里程计）
...
base_link → livox_frame (静态 TF，默认高度 0.15m + 前倾40°旋转补偿；v8.2 支持 lidar_* 参数现场微调)
```
...
>
> **v8.2**：`cartographer_2d_slam.launch.py` 默认关闭 STM32 TF 发布，Cartographer 接管 SLAM TF；STM32 `/odom` 仍可用于监控。后续只有在轮径/轮距/角速度方向和 covariance 标定完成后，才建议重新评估 `use_odometry=true`。
...
# v8.2 常用调参：收窄/放宽点云切片
ros2 launch czybot_slam cartographer_2d_slam.launch.py scan_min_height:=-0.20 scan_max_height:=0.35 scan_range_max:=15.0

# v8.2 对比测试：关闭 scan_dedup 人工改时间戳
...
| 地面/桌腿/门框上沿等多高度点投影导致双墙 | v8.2 默认窄切片 `-0.20m~0.35m`，必要时通过 `scan_min_height/scan_max_height` 现场调整 |
| 远距离稀疏点和反射点影响匹配 | v8.2 将 2D 建图 `range_max` 默认调为 `15.0m` |
| 雷达安装外参 | 静态 TF base_link→livox_frame 默认 `(0,0,0.15)` + q=`0,-0.342,0,0.940`，v8.2 支持 `lidar_*` launch 参数微调 |
| 未标定 STM32 里程计可能污染建图 | v8.2 `cartographer_2d.lua` 默认 `use_odometry=false`；完成 covariance 和底盘标定后再评估开启 |
| Ctrl-C 退出时 scan_dedup 报 `rcl_shutdown already called` | v8.2 增加 `if rclpy.ok(): rclpy.shutdown()` 保护 |
