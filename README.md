...
v13.0 在 v12.0 VLM 场景理解基础上，新增火灾/烟雾识别情景：`fire_smoke_node` 使用 CPU YOLOv5 对 D435i RGB 画面做火/烟一阶筛查，连续命中后发布 `/fire_smoke/prealert`；`vlm_node` 调用 Qwen-VL 对疑似画面做二次确认并发布 `/alert/fire`；后端把该 ROS2 topic 转为 WebSocket `fire_alert`；前端通过全局 `FireAlertOverlay` 在任意页面展示高危红色横幅、低危 toast、详情弹窗和 Web Audio 警报音。

当前火警数据链路：

```
D435i RGB
  → fire_smoke_node（CPU YOLOv5，连续 3 次命中触发）
  → /fire_smoke/prealert
  → vlm_node（Qwen-VL 二次确认，严格 JSON）
  → /alert/fire
  → backend ros2_bridge + data_pusher
  → WebSocket topic "fire_alert"
  → useFireAlert composable
  → FireAlertOverlay（横幅 + toast + 弹窗 + 警报音）
```

火警联调请优先看 [FIRE_SMOKE_GUIDE.md](FIRE_SMOKE_GUIDE.md)，里面包含终端 A~E 启动顺序、25 秒内验收标准、误报漏报调参表、常见报错对照表和一键关停说明。

### 本次会话新增/修改（v13.0）

前端 Stage 4：

- `frontend/src/composables/useFireAlert.ts`：新增模块级单例火警状态，自动订阅 WS `fire_alert` topic，并合成 Web Audio 警报音（high=4 声 880Hz，low=2 声 520Hz）。
- `frontend/src/components/FireAlertOverlay.vue`：新增三层告警 UI：高危顶部红色脉冲横幅、低危右下角 toast、详情 ElDialog。
- `frontend/src/App.vue`：在根节点挂载 `<FireAlertOverlay />`，保证任意页面均可收到和展示告警。
- `frontend/src/api/websocket.ts`：将 `fire_alert` 加入 `AUTO_SUBSCRIBED_TOPICS`，与后端 `DEFAULT_TOPICS` 对齐。

文档 Stage 5：

- `FIRE_SMOKE_GUIDE.md`：新增火灾/烟雾检测上手指南，包含架构图、启动顺序、联调测试、误报漏报调参、常见报错、关键文件速查和一键关停。

火/烟模型仓库整理：

- 本地目录从 `fire-smoke-detect-yolov4/` 重命名为 `fire-smoke-detect-yolov5/`，与实际使用的 YOLOv5 源码一致。
- 清理旧 YOLOv4、爬虫、结果、readme、xml 标注、`.git`、notebook、inference 等与当前部署无关内容。
- `fire_smoke_node.py`、`fire_smoke.launch.py`、`fire_smoke_params.yaml`、`FIRE_SMOKE_GUIDE.md` 的路径引用已统一改为 `fire-smoke-detect-yolov5/yolov5`。
- 操作指南里的 `cp` 命令已改为从 `fire-smoke-detect-yolov5/yolov5/best.pt` 复制权重。
...
│       │   ├── d435i_detection/fire_smoke_node.py # 火/烟检测节点（v13.0）
...
├── fire-smoke-detect-yolov5/ # 火/烟检测开源 YOLOv5 源码与 best.pt（v13.0 整理后保留）
...
完整联调按 [FIRE_SMOKE_GUIDE.md](FIRE_SMOKE_GUIDE.md) 执行。最小启动顺序如下：
...
验收：手机全屏播放火焰视频对准 D435i，25 秒内应看到终端 C `[FIRE] 预警发出 hits=3`、终端 D `[FIRE] 二次确认结果 level=high`、终端 E `[PUSH] fire_alert level=high`，浏览器出现顶部红色脉冲横幅、详情弹窗和 4 声 880Hz 警报音。低危 `level=low` 只显示右下角 toast，不阻塞操作。
...
`fire_smoke_node.py`、`fire_smoke.launch.py`、`fire_smoke_params.yaml` 和 `FIRE_SMOKE_GUIDE.md` 已同步使用该路径；旧的 `fire-smoke-detect-yolov4` 路径不再使用。
...
/fire_smoke/prealert
  → vlm_node（火警二次确认：fire_alert_cooldown_sec + 严格 JSON）
    → /alert/fire
      → ros2_bridge → data_pusher → WebSocket fire_alert
        → FireAlertOverlay 全局告警
```
...
v13.0 的火警能力采用“两段式检测”：YOLOv5 负责快速筛查，Qwen-VL 只在一阶命中后做二次确认，降低晚霞、红衣服、暖光灯等误报对前端的干扰。

### 数据流

```
D435i RGB
  → fire_smoke_node（CPU YOLOv5，连续 N 次命中）
    → /fire_smoke/prealert
      → vlm_node（Qwen-VL 二次确认）
        → /alert/fire（level/reason/recommendation/image_b64）
          → ros2_bridge._parse_fire_alert()
            → data_pusher.push_fire_alert()
              → WebSocket fire_alert
                → useFireAlert 单例
                  → FireAlertOverlay
```
...
| 角色 | 文件 |
|------|------|
| 火/烟 YOLOv5 源码 | `fire-smoke-detect-yolov5/yolov5/` |
| 火/烟权重源文件 | `fire-smoke-detect-yolov5/yolov5/best.pt` |
| 一阶检测节点 | `ros2_ws/src/d435i_detection/d435i_detection/fire_smoke_node.py` |
| 一阶检测配置 | `ros2_ws/src/d435i_detection/config/fire_smoke_params.yaml` |
| 一阶检测启动 | `ros2_ws/src/d435i_detection/launch/fire_smoke.launch.py` |
| VLM 火警参数 | `ros2_ws/src/vlm_scene/config/vlm_params.yaml` |
| VLM 二次确认 | `ros2_ws/src/vlm_scene/vlm_scene/vlm_node.py` |
| 后端 ROS2 桥 | `backend/app/services/ros2_bridge.py` |
| 后端 WS 推送 | `backend/app/services/data_pusher.py` |
| 前端告警状态 | `frontend/src/composables/useFireAlert.ts` |
| 前端告警 UI | `frontend/src/components/FireAlertOverlay.vue` |
| 上手指南 | `FIRE_SMOKE_GUIDE.md` |

### 前端告警行为

- `level=high`：顶部红色脉冲横幅 + 自动详情弹窗 + 4 声 880Hz 警报音。
- `level=low`：右下角黄色 toast + 2 声 520Hz 提示音，不自动弹窗。
- `level=none`：不展示 UI。
- `dismiss()` 按 `frame_id` 去重；同一帧点“我已知晓”后不再弹，新帧正常告警。
- `useFireAlert` 是模块级单例，加载时立即订阅 WS `fire_alert`，页面切换不会断流。

### 火/烟模型目录整理

当前工程只保留与 YOLOv5 火/烟检测有关的源码和权重。历史目录 `fire-smoke-detect-yolov4/` 已更名为 `fire-smoke-detect-yolov5/`，并删除以下无关内容：

```bash
yolov4/ result/ crawl/ readmes/ xml_lab/ .git/
latest_darknet_API.py LICENSE README.md .gitignore
yolov5/tutorial.ipynb yolov5/inference/
```

涉及路径引用的 4 个文件已经同步：

- `ros2_ws/src/d435i_detection/d435i_detection/fire_smoke_node.py`
- `ros2_ws/src/d435i_detection/launch/fire_smoke.launch.py`
- `ros2_ws/src/d435i_detection/config/fire_smoke_params.yaml`
- `FIRE_SMOKE_GUIDE.md`
