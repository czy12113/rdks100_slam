...
| 导航规划 | Nav2 阿克曼参数、SmacPlannerHybrid + MPPI Ackermann、目标点设置、多点巡逻（预留） |
...
`czybot_navigation2/config/nav2_params.yaml` 已按非全向、近似阿克曼运动约束更新，导航输出最终仍汇入 `/cmd_vel`，由 `stm32_bridge.py` 做最后一道限速、死区、超时和急停兜底。

### 启动

```bash
ros2 launch czybot_navigation2 navigation_real_robot.launch.py map:=/path/to/room.yaml
```
...
| MPPIController | `vx_max / wz_max / vy_max` | `0.60 / 1.20 / 0.0` |
| MPPIController | `AckermannConstraints.min_turning_r` | `0.45` |
| Planner | `planner_plugin` | `SmacPlannerHybrid` |
| Planner | `motion_model_for_search` | `DUBIN` |
| Planner | `minimum_turning_radius` | `0.45` |
| Behaviors | `behavior_plugins` | `drive_on_heading / wait / assisted_teleop` |
| velocity_smoother | `max_velocity` | `[0.60, 0.0, 1.20]` |
| velocity_smoother | `deadband_velocity` | `[0.005, 0.0, 0.010]` |
| collision_monitor | `cmd_vel_out_topic` | `/cmd_vel` |
...
RViz 给目标点后，路径应为平滑 Dubin 曲线，到点附近不应原地扭动；`/cmd_vel` 的角速度最大值不应超过 `1.20`。
...
导航控制：Nav2(Hybrid-A* + MPPI Ackermann) → velocity_smoother → collision_monitor → /cmd_vel → stm32_bridge → STM32
```
