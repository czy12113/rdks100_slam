#!/bin/bash
# STM32串口通信诊断脚本
# 在RDK S100上运行: bash diagnose_stm32.sh
...
# ── 步骤3：发送测试命令并监听响应 ──────────────────────────────────
echo -e "\n${YELLOW}[3/5] 向 /dev/ttyUSB0 发送速度命令并监听响应...${NC}"
echo -e "  协议格式: AA 55 01 [linear_int16_LE] [angular_int16_LE] 00 [checksum] 0D"
echo -e "  发送: 前进 200mm/s (AA 55 01 C8 00 00 00 C9 0D)"

if [ ! -e /dev/ttyUSB0 ]; then
    echo -e "  ${RED}✗ /dev/ttyUSB0 不存在，跳过${NC}"
else
    # 配置串口
    stty -F /dev/ttyUSB0 115200 raw -echo cs8 -cstopb -parenb 2>/dev/null

    # 构造帧: AA 55 01 C8 00 00 00 C9 0D
    # linear=200(0x00C8 LE), angular=0, reserved=0x00
    # checksum = (0x01 + 0xC8 + 0x00 + 0x00 + 0x00 + 0x00) & 0xFF = 0xC9
    CMD='\xAA\x55\x01\xC8\x00\x00\x00\xC9\x0D'

    echo -e "  正在发送命令..."
    printf "$CMD" > /dev/ttyUSB0

    echo -e "  等待STM32响应 (2秒)..."
    RESPONSE=$(timeout 2 cat /dev/ttyUSB0 | xxd | head -5)
    ...
    STOP='\xAA\x55\x01\x00\x00\x00\x00\x01\x0D'
    printf "$STOP" > /dev/ttyUSB0
    echo -e "  已发送停止命令"
fi

# ── 步骤4：监听STM32主动发送的数据 ──────────────────────────────────
echo -e "\n${YELLOW}[4/5] 监听 /dev/ttyUSB0 主动发送的数据 (3秒)...${NC}"
...
if [ -e /dev/ttyUSB0 ]; then
    ...
    DATA=$(timeout 3 cat /dev/ttyUSB0 | xxd | head -10)
    if [ -n "$DATA" ]; then
...
if ! command -v ros2 &>/dev/null; then
    ...
    for node in stm32_bridge tjc_hmi_bridge; do
...
echo "  1. 如果步骤3/4无响应 → STM32串口通信有问题，检查接线和固件"
echo "  2. 如果步骤3/4有响应 → STM32正常，问题在ROS节点"
echo "  3. 如果串口被占用 → 先停止占用进程再测试"
echo "  4. 尝试交换 ttyUSB0 和 ttyUSB1 的连接"
